...
import { getToken, setToken, clearToken, getDeviceId } from './storage';

// Re-export storage helpers so consumers can do `import { setToken } from './api'`
export { getToken, setToken, clearToken };

// In RN, use an absolute URL. Set via env or change for your server.
// For local dev with a real device, use your machine's LAN IP, e.g. 'http://192.168.x.x:8000'
const API_BASE = process.env.EXPO_PUBLIC_API_BASE ?? 'http://10.0.2.2:8000/api/v1';
