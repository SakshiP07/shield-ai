...
const LEVELS: { id: 'standard' | 'balanced' | 'high'; label: string; desc: string }[] = [
  { id: 'standard', label: 'Standard', desc: 'Fewer alerts; lower false positives' },
  { id: 'balanced', label: 'Balanced', desc: 'Recommended for most people' },
  { id: 'high', label: 'Strict', desc: 'Flag more suspicious activity' },
];
...
export function AiPreferencesPage() {
  ...
  const save = async (ai_sensitivity: 'standard' | 'balanced' | 'high') => {
    const previous = prefs ?? DEFAULT_PREFS;
    setPrefs({ ...previous, ai_sensitivity });
    setSaving(true);
    try {
      const saved = await api.updatePreferences({ ai_sensitivity });
      setPrefs(saved);
      setError('');
  ...
  return (
    <MobilePage style={styles.page}>
      <Text style={styles.description}>
        Controls how strict fraud detection is when SMS or scans are checked.
        {'\n\n'}
        Standard = fewer alerts{'\n'}
        Balanced = recommended{'\n'}
        Strict = more warnings for suspicious messages
      </Text>
      {saving ? <Text style={styles.saving}>Saving…</Text> : null}
      <MobileCard padding="sm">
        {LEVELS.map((level, index) => {
          const on = current.ai_sensitivity === level.id;
          return (
            <Pressable
              key={level.id}
